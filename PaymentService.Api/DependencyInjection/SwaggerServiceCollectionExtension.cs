...
namespace PaymentService.Api.DependencyInjection
{
    public static class SwaggerServiceCollectionExtension
    {
        public static IServiceCollection AddSwaggerWithJwt(this IServiceCollection services, IWebHostEnvironment env)
        {
            ...
            {
                options.SwaggerDoc("PaymentService", new OpenApiInfo
                {
                    Version = "v1",
                    Title = "Payment Service API",
                    Description = "Web API for Payment management in E-commerce app.",
                ...
                if (env.IsDevelopment())
                {
                    ...
                    options.AddServer(new OpenApiServer { Url = "/payment" });
                }
                else
                {
                    options.AddServer(new OpenApiServer { Url = "/payment" });
