...
namespace DeliveryService.Api.DependencyInjection
{
    public static class SwaggerServiceCollectionExtension
    {
        public static IServiceCollection AddSwaggerWithJwt(this IServiceCollection services, IWebHostEnvironment env)
        {
            ...
            {
                options.SwaggerDoc("DeliveryService", new OpenApiInfo
                {
                    Version = "v1",
                    Title = "Delivery Service API",
                    Description = "Web API for Delivery management in E-commerce app.",
                ...
                if (env.IsDevelopment())
                {
                    ...
                    options.AddServer(new OpenApiServer { Url = "/delivery" });
                }
                else
                {
                    options.AddServer(new OpenApiServer { Url = "/delivery" });
